...
--}}
<summary class="list-none cursor-pointer relative inline-flex items-center justify-center w-9 h-9 rounded-full hover:bg-surface-100">
    ...
    @if($unreadCount > 0)
        <span class="absolute -top-0.5 -right-0.5 bg-rejected-500 text-white text-[10px] rounded-full w-4 h-4 flex items-center justify-center">{{ $unreadCount > 9 ? '9+' : $unreadCount }}</span>
    @endif
</summary>
<div class="absolute right-0 mt-2 w-[30rem] max-w-[94vw] bg-white rounded-xl shadow-lg border border-surface-200 z-30">
